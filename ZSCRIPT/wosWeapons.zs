...
class wosWeapon : StrifeWeapon {
	...
	action void W_ShootFirearm3(int damageMultiplier, string weapsnd, string weapPuff) {
		if(player == null) {
			return;
		}
		player.mo.PlayAttacking2 ();
		...
		A_StartSound (weapsnd, CHAN_WEAPON); //default "weapons/assaultgun"
		LineAttack (ang, PLAYERMISSILERANGE, BulletSlope (), damage, 'Hitscan', weapPuff);	
		//take away ammo from magazine
		invoker.magazine--;
		A_AlertMonsters();
	}
